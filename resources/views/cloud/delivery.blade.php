...
<div class="filter-bar">
    <div class="search-wrap">
        ...
        <input class="search-input" x-model="deliverySearch" placeholder="Search order, customer, staff…"
            type="search">
    </div>
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <template x-for="f in ['all','assigned','picked_up','delivered','failed']" :key="f">
            <button @click="deliveryFilter=f"
                :class="deliveryFilter === f ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'"
                x-text="f==='all'?'All':f.replace('_',' ')"></button>
        </template>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;">
    <template
        x-for="[label,color,key] in [['Assigned','#6366F1','assigned'],['Picked Up','#F97316','picked_up'],['Delivered','#10B981','delivered'],['Failed','#EF4444','failed']]"
        :key="key">
        <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
            <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;"
                x-text="label"></div>
            <div style="font-size:26px;font-weight:800;margin-top:4px;" :style="'color:' + color"
                x-text="allDeliveries.filter(d=>d.status===key).length"></div>
        </div>
    </template>
</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            <template x-if="filteredDeliveries.length===0">
                <tr>
                    <td colspan="8" style="text-align:center;padding:40px;color:#94A3B8;">No deliveries found</td>
                </tr>
            </template>
            <template x-for="d in filteredDeliveries" :key="d.id">
                <tr>
                    <td style="font-size:13px;font-weight:700;color:#1E293B;" x-text="'#'+(d.order_number||d.order_id)">
                    </td>
                    <td>
                    ...
                    <td>
                        <div style="display:flex;gap:4px;">
                            <template x-if="d.status==='assigned'">
                                <button @click="updateDelivery(d.id,'picked_up')" class="btn btn-secondary btn-sm">Pick
                                    Up</button>
                            </template>
                            <template x-if="d.status==='picked_up'">
                                <button @click="updateDelivery(d.id,'delivered')"
                                    class="btn btn-primary btn-sm">Deliver</button>
                            </template>
