...
<div class="filter-bar">
  ...
  <div style="display:flex;gap:6px;flex-wrap:wrap;">
    <template x-for="f in ['all','pending','processing','ready','out_for_delivery','delivered','cancelled']" :key="f">
      <button @click="orderFilter=f" :class="orderFilter===f?'btn btn-primary btn-sm':'btn btn-secondary btn-sm'" x-text="f==='all'?'All':f"></button>
    </template>
  </div>
...
<div class="card">
  ...
  <table class="data-table">
    ...
    <tbody>
      <template x-if="filteredOrders.length===0">
        <tr><td colspan="9" style="text-align:center;padding:40px;color:#94A3B8;">No orders found</td></tr>
      </template>
      <template x-for="o in filteredOrders" :key="o.id">
        <tr>
          <td>
            <span style="font-size:13px;font-weight:700;color:#1E293B;" x-text="'#'+o.number.split('-').pop()"></span>
  ...
  <div style="padding:14px 20px;border-top:1px solid #EEF0F6;display:flex;justify-content:space-between;align-items:center;">
    <span style="font-size:13px;color:#64748B;"><span x-text="filteredOrders.length"></span> orders</span>
    <div style="display:flex;gap:6px;">
      <button class="btn btn-secondary btn-sm" @click="orderPage>1&&orderPage--" :disabled="orderPage<=1">‹ Prev</button>
      <span style="font-size:13px;color:#64748B;line-height:30px;padding:0 8px;">Page <span x-text="orderPage"></span></span>
  ...
  <div class="modal" style="max-width:520px;">
    ...
    <div class="modal-bdy" x-show="selectedOrder">
      ...
      <div style="background:#F8F9FD;border-radius:12px;padding:16px;margin-bottom:16px;">
        <div style="font-size:12px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px;">Customer</div>
      ...
      <div style="background:#F8F9FD;border-radius:12px;padding:16px;margin-bottom:16px;">
        <div style="font-size:12px;font-weight:700;color:#64748B;text-transform:uppercase;letter-spacing:.07em;margin-bottom:8px;">Summary</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
          <div><div style="font-size:11px;color:#94A3B8;">Total</div><div style="font-size:18px;font-weight:800;color:#0F172A;" x-text="fmt(selectedOrder?.total||0)"></div></div>
          <div><div style="font-size:11px;color:#94A3B8;">Paid</div><div style="font-size:18px;font-weight:800;color:#10B981;" x-text="fmt(selectedOrder?.paid||0)"></div></div>
          <div><div style="font-size:11px;color:#94A3B8;">Due</div><div style="font-size:18px;font-weight:800;color:#EF4444;" x-text="fmt(selectedOrder?.due||0)"></div></div>
        </div>
      </div>
      <div>
