...
<div class="filter-bar">
    <div style="display:flex;gap:6px;">
        <button @click="reportPeriod='daily'"
            :class="reportPeriod === 'daily' ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'">Daily</button>
        <button @click="reportPeriod='monthly'"
            :class="reportPeriod === 'monthly' ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'">Monthly</button>
    </div>
    <div style="display:flex;gap:8px;align-items:center;margin-left:auto;">
        <input class="form-input" type="date" x-model="reportDate" style="height:34px;font-size:13px;"
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
        ...
        <div class="card-body">
            ...
            <template x-for="[status,count] in Object.entries(dailyData.orders_by_status||{})" :key="status">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                    ...
                    <div
                        style="flex:1;margin:0 12px;height:8px;background:#F1F5F9;border-radius:99px;overflow:hidden;">
                        <div style="height:100%;background:linear-gradient(90deg,#6366F1,#8B5CF6);border-radius:99px;"
                            :style="'width:' + Math.round(count / Math.max(dailyData.total_orders, 1) * 100) + '%'"></div>
                    </div>
                    <span style="font-size:13px;font-weight:700;color:#1E293B;min-width:28px;text-align:right;"
                        x-text="count"></span>
                </div>
            </template>
        </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <template x-for="[method,amount] in Object.entries(dailyData.payments_by_method||{})"
                :key="method">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span
                            style="width:8px;height:8px;border-radius:50%;background:linear-gradient(135deg,#6366F1,#8B5CF6);flex-shrink:0;"></span>
                        <span style="font-size:13px;font-weight:500;color:#0F172A;text-transform:capitalize;"
                            x-text="method.replace(/_/g,' ')"></span>
                    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            <template x-if="topServices.length===0">
                <tr>
                    <td colspan="5" style="text-align:center;padding:40px;color:#94A3B8;">No service data</td>
                </tr>
            </template>
            <template x-for="(s,i) in topServices" :key="i">
                <tr>
                    ...
                    <td><span
                            style="background:#EEF2FF;color:#6366F1;border-radius:99px;padding:2px 10px;font-size:12px;font-weight:700;"
                            x-text="s.qty"></span></td>
                    <td style="font-size:14px;font-weight:700;" x-text="fmt(s.revenue)"></td>
                    <td style="font-size:13px;">
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div
                                style="flex:1;height:6px;background:#F1F5F9;border-radius:99px;overflow:hidden;min-width:80px;">
                                <div style="height:100%;background:linear-gradient(90deg,#6366F1,#8B5CF6);border-radius:99px;"
                                    :style="'width:' + Math.round(s.revenue / Math.max(topServices.reduce((a, b) => a + (b
                                        .revenue || 0), 0), 1) * 100) + '%'">
                                </div>
                            </div>
                            <span style="color:#94A3B8;font-size:11px;"
                                x-text="Math.round(s.revenue/Math.max(topServices.reduce((a,b)=>a+(b.revenue||0),0),1)*100)+'%'"></span>
                        </div>
                    </td>
                </tr>
            </template>
