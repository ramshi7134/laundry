...
<div class="filter-bar">
    ...
    <div style="display:flex;gap:6px;">
        <button @click="inventoryFilter='all'"
            :class="inventoryFilter === 'all' ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'">All Items</button>
        <button @click="inventoryFilter='low'"
            :class="inventoryFilter === 'low' ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'">
            ⚠ Low Stock
            <span
                style="background:#FEF3C7;color:#D97706;border-radius:99px;padding:0 6px;margin-left:4px;font-size:11px;"
                x-text="allInventory.filter(i=>isLow(i)).length"></span>
        </button>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:20px;">
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">Total
            Items</div>
        <div style="font-size:26px;font-weight:800;color:#6366F1;margin-top:4px;" x-text="allInventory.length"></div>
    </div>
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">Low
            Stock Alerts</div>
        <div style="font-size:26px;font-weight:800;color:#F59E0B;margin-top:4px;"
            x-text="allInventory.filter(i=>isLow(i)).length"></div>
    </div>
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">Out of
            Stock</div>
        <div style="font-size:26px;font-weight:800;color:#EF4444;margin-top:4px;"
            x-text="allInventory.filter(i=>i.quantity<=0).length"></div>
    </div>
</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            <template x-if="filteredInventory.length===0">
                <tr>
                    <td colspan="9" style="text-align:center;padding:40px;color:#94A3B8;">No items found</td>
                </tr>
            </template>
            <template x-for="item in filteredInventory" :key="item.id">
                <tr :style="isLow(item) ? 'background:#FFFBEB;' : ''">
                    ...
                    <td>
                        <span
                            :style="isLow(item) ? 'color:#D97706;font-weight:800;font-size:15px;' :
                    ...
                    <td>
                        <template x-if="item.quantity<=0">
                            <span
                                style="background:#FEE2E2;color:#EF4444;border-radius:99px;padding:3px 10px;font-size:11px;font-weight:700;">Out
                                of Stock</span>
                        </template>
                        <template x-if="isLow(item)&&item.quantity>0">
                            <span
                                style="background:#FEF3C7;color:#D97706;border-radius:99px;padding:3px 10px;font-size:11px;font-weight:700;">Low
                                Stock</span>
                        </template>
                        <template x-if="!isLow(item)&&item.quantity>0">
                            <span
                                style="background:#D1FAE5;color:#059669;border-radius:99px;padding:3px 10px;font-size:11px;font-weight:700;">In
                                Stock</span>
                        </template>
                    </td>
                    <td>
...
<div x-show="showAdjustStock" class="modal-bg" style="display:none;">
    ...
    <div class="modal" style="max-width:420px;">
        <div class="modal-hdr">
            ...
            <div class="modal-sub">
                Current quantity: <strong x-text="selectedItem?.quantity"></strong> <span
                    x-text="selectedItem?.unit||'pcs'"></span>
            </div>
        </div>
        <div class="modal-bdy">
            ...
            <div class="form-group">
                ...
                <input class="form-input" type="number" min="0" step="1" x-model="adjustQty"
                    placeholder="0">
            </div>
            <div class="form-group">
                <label class="form-label">Reason</label>
                <input class="form-input" type="text" x-model="adjustReason"
                    placeholder="e.g. Restocked, Damaged, Used in service…">
            </div>
            <div style="background:#F8F9FD;border-radius:10px;padding:12px;font-size:13px;color:#64748B;">
                <template x-if="adjustType==='add'">
                    New quantity: <strong style="color:#059669;"
                        x-text="(selectedItem?.quantity||0)+(+adjustQty||0)"></strong>
                </template>
                <template x-if="adjustType==='remove'">
                    New quantity: <strong style="color:#EF4444;"
                        x-text="Math.max(0,(selectedItem?.quantity||0)-(+adjustQty||0))"></strong>
                </template>
