...
<div class="filter-bar">
    <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <template x-for="cat in ['all','rent','utilities','salaries','supplies','maintenance','marketing','other']"
            :key="cat">
            <button @click="expenseFilter=cat"
                :class="expenseFilter === cat ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'"
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:20px;">
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">This
            Month</div>
        <div style="font-size:26px;font-weight:800;color:#EF4444;margin-top:4px;"
            x-text="fmt(allExpenses.reduce((s,e)=>s+(+e.amount||0),0))"></div>
    </div>
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">Today
        </div>
        <div style="font-size:26px;font-weight:800;color:#F97316;margin-top:4px;"
            x-text="fmt(allExpenses.filter(e=>e.date===new Date().toISOString().slice(0,10)).reduce((s,e)=>s+(+e.amount||0),0))">
        </div>
    </div>
    <div style="background:#fff;border-radius:14px;border:1px solid #EEF0F6;padding:16px 18px;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.07em;">Records
        </div>
        <div style="font-size:26px;font-weight:800;color:#6366F1;margin-top:4px;" x-text="filteredExpenses.length">
        </div>
    </div>
</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            <template x-if="filteredExpenses.length===0">
                <tr>
                    <td colspan="6" style="text-align:center;padding:40px;color:#94A3B8;">No expenses found</td>
                </tr>
            </template>
            <template x-for="e in filteredExpenses" :key="e.id">
                <tr>
                    ...
                    <td>
                        <span
                            style="border-radius:99px;padding:3px 10px;font-size:11px;font-weight:700;background:#EEF2FF;color:#6366F1;"
        ...
        <div class="modal-bdy">
            ...
            <div class="form-group">
                ...
                <select class="form-select" x-model="newExpense.category">
                    ...
                    <template
                        x-for="cat in ['rent','utilities','salaries','supplies','maintenance','marketing','other']"
                        :key="cat">
            ...
            <div class="form-group">
                ...
                <input class="form-input" type="number" min="0" step="0.01" x-model="newExpense.amount"
                    placeholder="0.00">
            </div>
            <div class="form-group">
                <label class="form-label">Description</label>
                <textarea class="form-input" rows="2" x-model="newExpense.description" placeholder="Optional notes…"
